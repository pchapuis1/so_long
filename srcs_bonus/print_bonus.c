...
/*   Updated: 2023/03/02 14:18:12 by pchapuis         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	place_moves(t_data *img)
{
	...
	place_bloc(img, 3, 0);
	place_bloc(img, 4, 0);
	mlx_string_put(img->mlx, img->mlx_win, 1, 10, 0xFFFFFF, "Moves: ");
	tmp = ft_itoa(img->count - 1);
	if (!tmp)
		return (1);
	mlx_string_put(img->mlx, img->mlx_win, 45, 10, 0xFFFFFF, tmp);
	free(tmp);
